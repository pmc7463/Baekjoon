...
* 1371 '가장 많은 글자' 문제
    + 문장이 주워지고
    + 가장 많이 쓰인 알파벳을 출력하는 문제이다.
+ gets()로 받았다.
+ 문장의 알파벳에서 - 'a' 를 하여 아스키 코드값은 카운터의 인덱스 값으로 이용한다.
```
    cnt[alpha[i] - 'a']++;
```
***
...
* 20254 'Site Score' 문제
  + 4개의 값을 입력 받는다.
  + 4개의 값은 다음 식에 입력값으로 들어간다.
  + 56UR + 24TR + 14UO + 6TO 이 식의 답을 출력하는 문제이다.
* 논문 보느라 문제 푸는것을 잊어버렸다.
* 이거 이제 그만 할까?
***

<br>

# 2023-11-25 Saturday(1)
* 14405 '피카츄' 문제
    + 'pi', 'ka', 'chu'를 이어 붙여서 피카츄를 만들수 있는지 없는지 판별하는 문제이다.
    + pikapi --> YES
    + pikachut --> NO
+ 각 자리에 첫글자 부터 마지막 글자 까지 맞는지 확인하였다.
```
else if (str[i] == 'c') {
    if (str[i + 1] == 'h') {
        if (str[i + 2] == 'u') {
...
* 1225 '이상한 곱셈' 문제     
    + 두 숫자가 주워진다.
    + 각 자리의 모든 조합의 수를 곱하고 전부 다 더하고 출력한다.
+ 반복문을 2개를 사용하여 모든 조합을 구한다.
+ 아스키 코드를 이용해서 정수 값을 저장한다.
```
...
# 2023-11-23 Thursday(1)
* 18408 '3 つの整数 (Three Integers)' 문제
    + A, B, C가 있고 이 들은 1, 2만 고를 수 있다.
    + 1과 2중에 더 많이 고른 쪽을 출력하는 문제이다.
+ 각 수를 더해서 값 별로 switch문을 이용해서 나눴다.
***

<br>

# 2023-11-22 Wednesday(1)
* 9653 '스타워즈 로고' 문제
    + 스타워즈 로고를 출력하면 되는 문제이다.
+ 오늘은 멀리 출장이 있어 쉬운문제로 푼다.
***
...
# 2023-11-14 Tuesday(1)
* 23971 'ZOAC 4' 문제
    + ![23971](./img/23971.png))
    + 세로, 가로, 세로 거리두기, 가로 거리두기 값이 주워진다.
    + 그림처럼 거리두기를 고려하여 최대 앉을 수 있는 사람의 수를 출력하는 문제이다.
+ 올림(거리 / (거리두기 + 1))을 이용하면된다.
```
    a = ceil(col / (x + 1));
    b = ceil(row / (y + 1));
```
+ 올림 함수를 사용할 때는 실수로 선언하하기
```
#include <math.h>
...
* 15740 'A + B - 9'문제
	+ 두 수를 더하는 문제이다.
+ C언어에서는 숫자가 커서 그냥 더하면 틀린다.
+ long long해도 75점이다
+ 시간이 없었어 파이썬으로 풀었다.
+ 오늘 다른 문제 풀다가 못 풀어서 급하게 이걸 풀었다.
***
...
* 2954 '창영이의 일기장' 문제
    + 이상한 문자열이 있다.
    + 이 문자열의 특별한 규칙이 있다.
    + 모음 뒤에 p를 붙이고 다시 모음을 하나 더 붙인다.
    + 예) e가 오면 epe 이렇게 문자열을 만든다.
    + 원래 문장으로 바끄어 출력하는 문제이다.
+ 모음을 리스트로 만든다.
```
vowels = ['a', 'e', 'i', 'o', 'u']
```
+ 문장 인덱스를 반복하면서 문장안에 모음이 있는지 확인한다.
+ 있다면 문자열 인덱스를 2칸 뛰어넘는다.
```
...
* 19602 'Dog Treats' 문제
    + 간식의 사이즈가 Small, Medium, Large이렇게 3 종류가 있다.
    + 각 사이즈에 맞는 수량을 나눠준다.
    + 1 × S + 2 × M + 3 × L > 10? happy : sad 출력하면된다.
***
...
* 5691 '평균 중앙값 문제' 문제
    ...
    + A와 B는 주워진다. 
    + (A+B+C) / 3을 해서 평균값과 중앙에 있는 A값을 같게 만드는 가장 작은 정수 C를 찾아야 한다.
+ C를 구하는 방법
```
c = a * 3 - a - b;
```
...
# 2023-11-08 Wednesday(1)
* 10812 '바구니 순서 바꾸기' 문제
    +  바구니의 순서를 회전시킬 때, 순서를 회전시킬 범위를 정하고, 그 범위 안에서 기준이 될 바구니를 선택하는 문제이다.

+ 가운데 부터 끄까지 값을 가장앞으로 땡겨서 풀었다.
```
...
# 2023-11-07 Tuesday(1)
* 23037 '5의 난수' 문제
    + 5자리를 가진 수가 주워진다.
    + 각 자리 수를 5의 제곱을 하여 누적합을 더하는 문제이다.
+ 제곱 함수 pow()을 쓰기 위해서 math.h 를 활용
```
#include <math.h>
...
* 2754 '학점계산' 문제
    + 문자로 학점이 주워지면
    + 숫자로 얼마인지 출력하는 문제이다.
+ switch - case문을 활용해서 풀었다.
+ 코드가 너무 긴데 줄일수 있을것 같은데 떠오르지가 않는다.
***

<br>

# 2023-11-05 Sunday(1)
* 20976 '2 番目に大きい整数 (The Second Largest Integer)' 문제
    + 3개의 숫자 중에 2번째로 큰 숫자를 출력하는 문제이다.
+ 버블 정렬하여 2번째 수를 출력하였다.
```
    temp = arr[0];
...
# 2023-11-04 Saturday(1)
* 5724 '파인만' 문제
    + 한 변이 N인 정사각형에서 정사각형을 최대 몇개 까지 그릴수 있는지 알아보는 문제이다.
+ N에서 1까지 제곱을 하여 누적합을 구하면 값이 나온다.
+ do while문을 사용해보았다.
+ 잘 안쓰는 코드이다.
```
do {
...
# 2023-11-03 Friday(1)
* 10822 '더하기' 문제
    + 예를 들어 입력값으로 아래와 같이 올 때
    + 10,20,30,50,100
    + 이 문자열에 들어있는 값을 전부 더해서 출력하는 문제이다.
+ 파이썬의 split(",")를 활용하여 풀었다.
+ C언어는 어떻게 접근해야 할까?
+ 다음에 도전해봐야겠다.
***
...
# 2023-11-02 Thuesday(1)
* 2702 '초6 수학' 문제
    + 두 수가 주워질때 
    + 최소공배수와 최대공약수를 구하는 문제이다.
+ 최대 공약수를 재귀함수로 구하였다.
```
int gcd(int a, int b) {
    if (b == 0)
...
```
+ 최소공배수는 두 수 곱하고 최대 공약수를 나누면 값이 나온다.
***
